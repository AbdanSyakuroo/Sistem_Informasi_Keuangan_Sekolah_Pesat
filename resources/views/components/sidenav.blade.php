...
<head>
    ...
    background-color: #f0f0f0; /* warna latar aktif */
    font-weight: bold;
    color: #000 !important;
    border-radius: 6px;
    width: 100%; /* pastikan full container */
    display: block; /* pastikan blok penuh */
}

.sidebar-nav-wrapper .nav-item.active > a {
    background-color: #e9ecef; /* warna untuk parent dropdown aktif */
}

.dropdown-nav a {
    display: block; /* bikin dia full 1 baris */
    width: 100%; /* pastikan lebar penuh */
    padding: 8px 16px; /* atur padding biar rapi */
    box-sizing: border-box; /* biar padding ga bikin melar */
}
...
.dropdown-nav a.active {
    ...
    font-weight: bold;
    color: #000;
    border-radius: 8px; /* kalau mau rounded */
}
        ...
        <nav class="sidebar-nav">
            <ul>
                ...
                <li class="nav-item nav-item-has-children {{ $laporan_active ? 'active' : '' }}">
                    <a href="#0" class="{{ $laporan_active ? '' : 'collapsed' }}" data-bs-toggle="collapse" data-bs-target="#ddmenu_2" aria-expanded="{{ $laporan_active ? 'true' : 'false' }}">
                        <span class="icon">
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <ul id="ddmenu_2" class="collapse dropdown-nav {{ $laporan_active ? 'show' : '' }}">
                        <li>
                            <a href="{{ url('laporan') }}" class="{{ Request::is('laporan') ? 'active' : '' }}">Keseluruhan</a>
                        </li>
                        <li>
                            <a href="{{ url('pengeluarans/filter') }}" class="{{ Request::is('pengeluarans/filter') ? 'active' : '' }}">By Sumber Dana</a>
                        </li>
                    </ul>
                </li>
                ...
                {{-- Sumber Dana --}}
                <li class="nav-item">
                    <a href="{{ url('sumber_dana') }}" class="{{ Request::is('sumber_dana') ? 'active' : '' }}">
                        <span class="icon">
                            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <li class="flex items-center">
                    <form method="POST" action="{{ route('logout') }}" class="w-50 ml-20">
                        @csrf
                        <button type="submit" class="btn btn-danger w-100 rounded-md d-flex align-items-center justify-content-center py-2 px-4 shadow-sm">
                            Logout
                        </button>
                    </form>
                </li>
